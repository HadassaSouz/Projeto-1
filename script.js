...
function pace () {
    ...
    if (nome !== '' && distancia !== '' && peso !== '' && tempo !== '') {
        ...
        const valorPACE = ((tempo_minutos)/distancia).toFixed(2);
        
        const calorias = (velocidade*peso*0.0175*(tempo_minutos)).toFixed(2);
    

        resultado.textContent = `Olá, ${nome}! Você percorreu ${distancia}km em ${horas}h${minutos}min${segundos}s, o que implica em uma velocidade média de ${velocidade}km/h, portanto seu pace é ${valorPACE}km/min. Estima-se que você tenha consumido ${calorias}calorias.`;
